...
ok(
  "other distributions fail explicitly",
  /Automatic Node installation is supported on Debian and Ubuntu only/.test(source),
);
ok(
  "NodeSource 24-style repository is configured",
  /deb\.nodesource\.com\/node_\$\{MIN_NODE_MAJOR\}\.x nodistro main/.test(source),
);
ok(
  "NodeSource repository is pinned to its signing key",
  /signed-by=\/etc\/apt\/keyrings\/nodesource\.asc/.test(source),
);
ok(
  "the NodeSource signing key is downloaded as data",
  /nodesource-repo\.gpg\.key -o "\$key_tmp"/.test(source),
);
...
ok(
  "Node is re-checked after bootstrapping",
  /install_node24[\s\S]*NODE_MAJOR="\$\(node_major\)"/.test(source),
);
ok(
  "npm is still required before installing Levix",
  /command -v npm[\s\S]*npm install -g "\$\{PACKAGE\}@\$\{VERSION\}"/.test(source),
);
